...
function patchRequest(body: unknown) {
  return new NextRequest("http://localhost/api/obras/some-id/presupuesto", {
    method: "PATCH",
...
describe("PATCH /api/obras/:id/presupuesto", () => {
  ...
  it("should return 200 with the updated obra on a valid budget", async () => {
    if (!hasDatabaseUrl) {
      console.log("SKIP: no DATABASE_URL configured — nothing modified.");
      return;
    }

    const response = await PATCH(patchRequest({ presupuestoInicial: 3000000 }), {
      params: Promise.resolve({ id: obraId }),
    });
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body.presupuestoInicial).toBe(3000000);
  });

  it("should return 400 with field errors on an invalid budget", async () => {
    if (!hasDatabaseUrl) {
      console.log("SKIP: no DATABASE_URL configured — nothing modified.");
      return;
    }

    const response = await PATCH(patchRequest({ presupuestoInicial: 0 }), {
      params: Promise.resolve({ id: obraId }),
    });
    const body = await response.json();
    ...
    expect(body.fields).toHaveProperty("presupuestoInicial");
  });
  ...
  it("should return 404 when the obra does not exist", async () => {
    ...
    const response = await PATCH(patchRequest({ presupuestoInicial: 1000 }), {
